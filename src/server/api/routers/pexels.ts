		...
		.query(async ({ input }): Promise<PexelsCollectionResponse> => {
			...
			try {
				...
				if (!response.ok) {
					...
					throw new Error(
						`Error fetching from Pexels (search): ${response.status} ${response.statusText}`,
					);
				}
				const data = (await response.json()) as PexelsCollectionResponse;
				return data;
			} catch (error) {
		...
		.query(async ({ input }): Promise<PexelsCollectionResponse> => {
			...
			try {
				...
				if (!response.ok) {
					...
					throw new Error(
						`Error fetching from Pexels (curated): ${response.status} ${response.statusText}`,
					);
				}
				const data = (await response.json()) as PexelsCollectionResponse;
				return data;
		...
		.query(async ({ input }): Promise<PexelsPhoto> => {
			...
			try {
				...
				if (!response.ok) {
					...
					throw new Error(
						`Error fetching from Pexels (photo by ID): ${response.status} ${response.statusText}`,
					);
				}
